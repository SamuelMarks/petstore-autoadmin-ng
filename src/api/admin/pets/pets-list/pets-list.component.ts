...
export class PetListComponent implements AfterViewInit {
    ...
    executeCollectionAction(action: any): void {
        ...
        switch (action.methodName) {
            case 'updatePet': this.svc.updatePet({} as any).subscribe({ next: () => { this.snackBar.open('Action successful.', 'OK', { duration: 3000 }); }, error: (e) => this.snackBar.open('Action failed.', 'OK', { duration: 5000 }) }); break;
            case 'findPetsByTags': this.svc.findPetsByTags({} as any).subscribe({ next: () => { this.snackBar.open('Action successful.', 'OK', { duration: 3000 }); }, error: (e) => this.snackBar.open('Action failed.', 'OK', { duration: 5000 }) }); break;
